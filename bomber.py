...
def do_zip_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("Evil-sms was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update Evil-sms.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/Hk4crprasad/Evil-sms.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/Hk4crprasad/Evil-sms.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
            "https://raw.githubusercontent.com/Hk4crprasad/Evil-sms/master/.version"
            ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("Evil-sms is up-to-date")
        mesgdcrt.GeneralMessage("Starting Evil-sms")


def notifyen():
    try:
        noti = requests.get(
            "https://raw.githubusercontent.com/Hk4crprasad/Evil-sms/master/.notify"
            ).text.upper()
        if len(noti) > 10:
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit = {"sms": 500000000, "call": 1500000, "mail": 200000000}
        cc, target = "", ""
        if mode in ["sms", "call"]:
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("Evil-sms will work only in Python v3")
    sys.exit()
...
description = """Evil-sms - Your Friendly Spammer Application

Evil-sms can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

Evil-sms is not intented for malicious uses.
"""
...
parser.add_argument("-call", "--call", action="store_true",
                    help="start Evil-sms with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start Evil-sms with MAIL Bomb mode")
parser.add_argument("-u", "--update", action="store_true",
                    help="update Evil-sms")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current Evil-sms contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current Evil-sms version")
